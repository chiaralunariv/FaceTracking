...
######
#Chiara Luna Rivolta (c.rivolta@bcbl.eu)
#Postdoctoral researcher @BCBL
#
#This script is adapted from the LipReading repository by Marco A. Flores-Coronado
#
#
...
# -it analizes videos of people speaking/gesturing or signing
...
while readvideo==True:
    ...
    if ret==False:
        ...
        break   
    gray_og=cv.cvtColor(frame, cv.COLOR_BGR2GRAY) #original frame for showing and saving output video
    gray =cv.cvtColor(frame, cv.COLOR_BGR2GRAY) #frame to be blacked out and used for tracking
    gray[350:700, 0:900] = 0 #black out lower half of the video so that tracking is only looking at the face
    ...
    for (x,y,z,w) in faces:
        ...
        color=(255,0,0)
        stroke=1
        cv.rectangle(gray_og,(x,y),(end_coord_x,end_coord_y),color,stroke)
        ...
        my_coord_magnitudes=[]
        if forgevideo==True:
            gray4saving=cv.cvtColor(gray_og,cv.COLOR_GRAY2BGR)
